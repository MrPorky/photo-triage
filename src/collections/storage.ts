...
import type { StandardSchemaV1 } from 'node_modules/zod/v4/core/standard-schema.d.cts';
 ...
 */
export type ClearStorageFn = () => Promise<void> | void;

/**
 * Type for the getStorageSize utility function
 */
export type GetStorageSizeFn = () => Promise<number> | number;
...
> & {
  ...
  const wrappedOnInsert = async (params: InsertMutationFnParams<any>) => {
    ...
    // Save to storage
    await saveToStorage(lastKnownData);
...
function createLocalStorageSync<T extends object>(
  ...
  const processStorageChanges = async () => {
    ...
    // Load the new data
    const newData = await loadFromStorage<T>(storageKey, storage, parser);
  ...
  const syncConfig: SyncConfig<T> & {
    ...
    sync: (params: Parameters<SyncConfig<T>[`sync`]>[0]) => {
      ...
      // Initial load
      void (async () => {
        const initialData = await loadFromStorage<T>(
          storageKey,
          storage,
          parser,
        );
